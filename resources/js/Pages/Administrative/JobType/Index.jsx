...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.jobTypeList");
    const { jobTypes, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: t("jobType.deleteTitle"),
            icon: <ExclamationCircleFilled />,
            content: t("jobType.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.job-type.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (jobTypes || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("job_type_edit") && (
                        <Edit key={record.id} jobType={record} />
                    )}
                    {hasPermission("job_type_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("jobType.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/job-type",
            title: (
                <>
                    <RiBriefcaseLine /> {t("breadcrumb.jobTypesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchJobType")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("job_type_create") && <Create />}
                        </Col>
                    </Row>
