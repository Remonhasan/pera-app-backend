...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.noticeList");
    const { notices, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            content: t("notice.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.notice.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (notices || []).filter(matchesSearch);
    ...
    const columns = [
        {
            ...
            sortOrder:
                state.sortedInfo?.columnKey === "title"
                    ? state.sortedInfo.order
                    : null,
        },
        {
            title: t("notice.colDescription"),
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("notice_edit") && (
                        <Edit key={record.id} notice={record} />
                    )}
    ...
    const breadcrumbItems = [
        ...
        },
        {
            ...
            title: (
                <>
                    <RiNotification3Line /> {t("breadcrumb.noticesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchNotice")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("notice_create") && <Create />}
                        </Col>
                    </Row>
