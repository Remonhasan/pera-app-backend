...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.savingTypeList");
    const { savingTypes, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: t("savingType.deleteTitle"),
            icon: <ExclamationCircleFilled />,
            content: t("savingType.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.saving-type.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (savingTypes || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("saving_type_edit") && (
                        <Edit key={record.id} savingType={record} />
                    )}
                    {hasPermission("saving_type_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("savingType.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/saving-type",
            title: (
                <>
                    <RiSafe2Line /> {t("breadcrumb.savingTypesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchSavingType")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("saving_type_create") && <Create />}
                        </Col>
                    </Row>
