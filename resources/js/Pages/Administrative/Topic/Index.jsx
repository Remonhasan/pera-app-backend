...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.topicList");
    const { topics, subjects, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            content: t("topic.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.topic.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (topics || []).filter(matchesSearch);
    ...
    const columns = [
        {
            title: t("topic.colSubject"),
            key: "subject",
            width: 200,
            sorter: (a, b) =>
                (a.subject?.name || "").localeCompare(b.subject?.name || ""),
            sortOrder:
                state.sortedInfo?.columnKey === "subject"
                    ? state.sortedInfo.order
                    : null,
            render: (_, record) => record.subject?.name || "—",
        },
        {
            title: t("topic.colTopic"),
            dataIndex: "topic",
            key: "topic",
            width: 240,
            sorter: (a, b) => (a.topic || "").localeCompare(b.topic || ""),
            sortOrder:
                state.sortedInfo?.columnKey === "topic"
                    ? state.sortedInfo.order
                    : null,
        },
        {
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/topic",
            title: (
                <>
                    <RiBookmarkLine /> {t("breadcrumb.topicsList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchTopic")}
