...
function buildFormState(goal) {
    return {
        ...
        bank_id: goal.bank_id ?? null,
        saving_type_id: goal.saving_type_id ?? null,
        start_date: goal.start_date ?? null,
        end_date: goal.end_date ?? null,
        amount: goal.amount ?? 0,
        description: goal.description ?? "",
        drive_link: goal.drive_link ?? null,
        status: Boolean(goal.status),
        goal_status: goal.goal_status ?? "pending",
    };
}

export default function Edit({ goal, members, banks, savingTypes }) {
    const { t } = useAdminT();
    const initial = useMemo(() => buildFormState(goal), [goal]);
    ...
    const showLoading = () => {
        const next = buildFormState(goal);
        setData(() => next);
        setOpen(true);
        setLoading(true);
        setTimeout(() => {
            form.setFieldsValue({
                ...next,
                start_date: next.start_date ? dayjs(next.start_date) : null,
                end_date: next.end_date ? dayjs(next.end_date) : null,
            });
            setLoading(false);
        }, 0);
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            put(route("administrative.goal.update", goal.id), {
                preserveScroll: true,
