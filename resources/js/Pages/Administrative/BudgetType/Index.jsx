...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.budgetTypeList");
    const { budgetTypes, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: t("budgetType.deleteTitle"),
            icon: <ExclamationCircleFilled />,
            content: t("budgetType.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.budget-type.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (budgetTypes || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("budget_type_edit") && (
                        <Edit key={record.id} budgetType={record} />
                    )}
                    {hasPermission("budget_type_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("budgetType.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/budget-type",
            title: (
                <>
                    <RiWallet3Line /> {t("breadcrumb.budgetTypesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchBudgetType")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("budget_type_create") && <Create />}
                        </Col>
                    </Row>
