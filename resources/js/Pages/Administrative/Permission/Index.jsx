...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.permissionList");
    ...
    const showDeleteConfirm = (record) => {
        confirm({
          ...
          content: t("permission.deleteContent"),
          okText: t("common.yes"),
          okType: 'danger',
          cancelText: t("common.no"),
          onOk() {
                destroy(route('administrative.permission.destroy', record.id));
          },
          onCancel() {
            // console.log('Cancel');
          },
    ...
    const filteredData = permissions
        .filter((permission) => {
            ...
            const label = permission.label ? permission.label.toLowerCase() : "";

            return (
                name.includes(state.searchText) ||
                label.includes(state.searchText)
            );
        })
        .slice(
                ...
                <Space>
                    ...
                    {hasPermission("permission_delete") && (
                        <Button
                            ...
                            icon={<DeleteOutlined />}
                            title={t("permission.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/permission",
            title: (
                <>
                    <LockOutlined /> {t("breadcrumb.permissionsList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: 'right' }}>
                           {hasPermission("permission_create") && <Create />}
                        </Col>
                    </Row>
                    ...
                    <Table
                        ...
                        pagination={{
                            ...
                            total: permissions.filter((permission) => {
