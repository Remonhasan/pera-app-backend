...
export default function Edit({ task, members }) {
    const { t } = useAdminT();
    const initial = useMemo(() => buildFormState(task), [task]);
    ...
    const showLoading = () => {
        const next = buildFormState(task);
        setData(() => next);
        setOpen(true);
        setLoading(true);
        setTimeout(() => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            put(route("administrative.task.update", task.id), {
                preserveScroll: true,
