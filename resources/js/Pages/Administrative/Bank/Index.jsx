...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.bankList");
    const { banks, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            ...
            content: t("bank.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.bank.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (banks || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("bank_edit") && (
                        <Edit key={record.id} bank={record} />
                    )}
                    {hasPermission("bank_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("bank.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/bank",
            title: (
                <>
                    <RiBankLine /> {t("breadcrumb.banksList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchBank")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("bank_create") && <Create />}
                        </Col>
                    </Row>
