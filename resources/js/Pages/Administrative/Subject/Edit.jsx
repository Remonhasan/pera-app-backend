...
function buildFrombank(subject) {
    return {
        name: subject.name ?? "",
        status: Boolean(subject.status),
    };
}

export default function Edit({ subject }) {
    const { t } = useAdminT();
    const initial = useMemo(() => buildFrombank(subject), [subject]);
    ...
    const showLoading = () => {
        const next = buildFrombank(subject);
        setData(() => next);
        setOpen(true);
        setLoading(true);
        setTimeout(() => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            put(route("administrative.subject.update", subject.id), {
                preserveScroll: true,
    ...
    return (
        <div>
            <Button
                ...
                icon={<EditOutlined />}
                onClick={showLoading}
                title={t("subject.editTitle")}
            />

            <Drawer
                closable
                destroyOnClose
                title={<p>{t("subject.editTitle")}</p>}
                placement="right"
                open={open}
                loading={loading}
                size="large"
            ...
            <style>{`
                ...
                .subject-form-input.ant-input:hover {
                    border-color: #1e3a5f !important;
                }
                .subject-form-input.ant-input:focus,
                .subject-form-input.ant-input-focused {
                    border-color: #1e3a5f !important;
