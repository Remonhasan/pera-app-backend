...
function buildFromBudget(budget) {
    return {
        user_id: budget.user_id ?? budget.user?.id ?? null,
        budget_type_id:
            budget.budget_type_id ?? budget.budget_type?.id ?? null,
        month: budget.month != null ? Number(budget.month) : dayjs().month() + 1,
        year: budget.year != null ? Number(budget.year) : dayjs().year(),
        date: budget.date ? String(budget.date).slice(0, 10) : null,
        amount: budget.amount != null ? Number(budget.amount) : 0,
        status: Boolean(budget.status),
    };
}

export default function Edit({ budget, budgetTypes, members }) {
    const { t } = useAdminT();
    ...
    const initial = useMemo(() => buildFromBudget(budget), [budget]);
    ...
    const showLoading = () => {
        const next = buildFromBudget(budget);
        setData(() => next);
        setOpen(true);
        setLoading(true);
        setTimeout(() => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            put(route("administrative.budget.update", budget.id), {
                preserveScroll: true,
