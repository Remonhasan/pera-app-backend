...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.expenseTypeList");
    const { expenseTypes, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: t("expenseType.deleteTitle"),
            icon: <ExclamationCircleFilled />,
            content: t("expenseType.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.expense-type.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (expenseTypes || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("expense_type_edit") && (
                        <Edit key={record.id} expenseType={record} />
                    )}
                    {hasPermission("expense_type_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("expenseType.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/expense-type",
            title: (
                <>
                    <RiMoneyDollarCircleLine /> {t("breadcrumb.expenseTypesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchExpenseType")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("expense_type_create") && <Create />}
                        </Col>
                    </Row>
