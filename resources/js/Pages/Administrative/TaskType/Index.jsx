...
export default function Index() {
    const { t } = useAdminT();
    const title = t("pages.taskTypeList");
    const { taskTypes, auth } = usePage().props;
    const { confirm } = Modal;
    ...
    const showDeleteConfirm = (record) => {
        confirm({
            title: t("taskType.deleteTitle"),
            icon: <ExclamationCircleFilled />,
            content: t("taskType.deleteContent"),
            okText: t("common.yes"),
            okType: "danger",
            cancelText: t("common.no"),
            onOk() {
                destroy(route("administrative.task-type.destroy", record.id));
            },
        });
    };
    ...
    const filtered = (taskTypes || []).filter(matchesSearch);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    {hasPermission("task_type_edit") && (
                        <Edit key={record.id} taskType={record} />
                    )}
                    {hasPermission("task_type_delete") && (
                        <Button
                            variant="outlined"
                            danger
                            onClick={() => showDeleteConfirm(record)}
                            icon={<DeleteOutlined />}
                            title={t("taskType.deleteBtnTitle")}
                        />
                    )}
                </Space>
            ),
    ...
    const breadcrumbItems = [
        ...
        {
            href: "/administrative/task-type",
            title: (
                <>
                    <RiListCheck2 /> {t("breadcrumb.taskTypesList")}
                </>
            ),
        },
    ];
                ...
                <div>
                    ...
                    >
                        <Col xs={24} sm={12} md={8}>
                            <Input
                                placeholder={t("common.searchTaskType")}
                        ...
                        <Col xs={24} sm={12} md={6} style={{ textAlign: "right" }}>
                            {hasPermission("task_type_create") && <Create />}
                        </Col>
                    </Row>
